...
- If aggregate residual coverage improves and the 100% goal remains unmet, continue aggregate attempts. Do not stop on a fixed attempt count while coverage is still increasing.
- Stop below 100% only when two consecutive measured coding-agent attempts show no coverage increase compared with the immediately previous attempt, and never run more than 10 coding-agent residual attempts for a target/run without reporting a policy blocker.
...
- For each residual target function, the coding agent must keep running the residual repair loop while the 100% goal is unmet and the latest attempt increased coverage. Each attempt must change a generated harness, fixture, stub, or testcase input and then recompile/replay/remeasure.
- Do not spend residual attempts on global reruns that do not target a specific uncovered function or branch. Each attempt must name the target function, intended gap, changed generated artifact, replay command, before/after coverage, and stop reason.
- Running the same generated binary or harness with different command-line arguments is not a new coding-agent repair attempt unless a distinct generated artifact snapshot/hash is recorded for that attempt.
- Record residual attempt history as JSON at `mcp_reports/coding_agent_residual_attempt_history.json` before final reporting. The file must contain either `finalCoverageGoalReached: true` or measured attempt records proving two consecutive no-increase generated-artifact repair attempts below 100%.
- Manual residual history entries must use MCP-verifiable keys. Each aggregate attempt must include `attempt`, `changedArtifact`, `coverageArtifact` or `measurementArtifact`, and `afterCoverage`. `coverageArtifact` must point to a parseable `_llvm.json`, `coverage*.json`, or `.info` file from the current run. Plain `*_report.txt`, `.profraw`, `.profdata`, or hand-entered `afterCoverage` values alone do not satisfy the final evidence gate.
- When one aggregate residual attempt sequence applies to multiple residual targets, connect it explicitly from `perFunction` using `attemptsRef: "aggregate"` or `aggregateAttemptRefs`. Each target below 100% must also have a classified stop reason such as `max-coverage`, `infeasible`, `crash-risk`, or `toolchain-blocked`; examples include unreachable branches and crash-risk probes.
- If `llvm-cov export` contains both generated harness files and the original source file, include `sourceFile` or `targetSourceFiles` in the residual history so the gate can select the original source-file summary instead of a harness summary.
...
- If the gate returns `coverage_still_increasing_retry_required`, `coverage_no_improvement_streak_below_required`, `residual_attempts_not_distinct_generated_artifact_changes`, or `coverage_growth_attempts_missing`, continue or repair the generated-artifact retry loop. Do not replace this with a standalone summary HTML.
...
- Coding-agent residual retry evidence: target function, attempt count, before coverage, after coverage, coverage delta, best coverage, changed generated artifacts, replay command, and stop reason. If any attempt increased coverage but final cumulative coverage is still below 100%, the report is incomplete unless later attempts prove two consecutive no-increase measurements or the 100% goal is reached.
